...
#         prompt_text = """
# 请根据我上传的图片，生成一份结构化中文JSON描述文件，用于AI图像生成。JSON需按以下要求进行**精细化字段拆分**，覆盖图片所有维度，无任何细节遗漏：

# 1.  基础元数据（必填字段：`basic_meta`）
#     - `image_ratio`：画面比例，精确到具体数值（如 16:9 / 4:3 / 1:1 / 9:16 / 2.35:1 电影宽屏）
#     - `estimated_resolution`：估算分辨率，标注像素值（如 1920×1080 / 512×512 / 3840×2160 4K）
#     - `file_quality`：画质特征（如 8K超高清 / 1080P高清 / 胶片颗粒质感 / 无噪点 / 轻微复古模糊）

# 2.  画面主体内容（必填字段：`main_content`，拆分为最小描述单元）
#     - `core_subject`：核心主体（如 一名穿JK制服的少女 / 一只橘猫 / 一座复古蒸汽朋克工厂 / 一片星空下的湖泊）
#     - `subject_details`：主体细节，逐条罗列
#       - 人物类：五官特征、发型发色、服饰款式/颜色/材质、姿态动作、表情神态、配饰（如眼镜/项链）
#       - 物体类：形状、颜色、材质纹理（如 木质粗糙纹理 / 金属反光质感）、磨损程度、摆放位置
#       - 场景类：空间结构（如 室内客厅 / 户外森林 / 科幻太空舱）、核心建筑/景观位置
#     - `background_elements`：背景元素，逐条罗列（如 远处的山脉 / 墙上的复古海报 / 漂浮的太空陨石 / 地面的落叶）
#     - `foreground_elements`：前景元素，逐条罗列（如 画面下方的一杯咖啡 / 窗前的白色窗帘 / 脚下的石板路）
#     - `interaction_relation`：主体与环境的互动关系（如 少女坐在窗边看书 / 橘猫趴在沙发上晒太阳 / 工厂烟囱冒出黑烟）

# 3.  艺术风格与氛围（必填字段：`art_style`，细分到具体流派和表现手法）
#     - `style_type`：核心艺术风格（如 日系二次元赛璐璐 / 新中式水墨写实 / 梵高印象派油画 / 赛博朋克像素风 / 美式3D卡通渲染）
#     - `color_scheme`：色彩方案（如 高饱和撞色 / 低饱和莫兰迪色调 / 暖黄色复古色调 / 冷蓝色科技色调 / 黑白灰单色）
#     - `light_effect`：光影效果，逐条拆分
#       - 光源类型（如 自然光 / 室内暖光灯 / 霓虹灯 / 月光）
#       - 光照方向（如 左侧侧光 / 顶光 / 逆光 / 漫射光）
#       - 光影强度（如 硬阴影对比强烈 / 柔和阴影无明显边界 / 高光过曝效果）
#     - `atmosphere`：画面氛围（如 温馨治愈 / 冷峻压抑 / 未来科技感 / 复古怀旧 / 梦幻唯美）
#     - `brush_texture`：笔触/质感（如 细腻平滑无笔触 / 粗犷油画笔触 / 水彩晕染质感 / 像素块颗粒感）

# 4.  构图与视角（必填字段：`composition`）
#     - `shooting_angle`：拍摄视角（如 平视 / 俯视 / 仰视 / 45°斜角 / 广角鱼眼）
#     - `composition_type`：构图方式（如 居中对称构图 / 三分法构图 / 对角线构图 / 框架式构图 / 留白构图）
#     - `focus_area`：焦点区域（如 主体人物面部 / 物体的中心位置 / 画面左侧的红色建筑）

# 5.  AI绘图适配补充（必填字段：`ai_prompt_supplement`）
#     - `positive_keywords`：正向关键词，提炼核心特征（如 高清细节 / 细腻皮肤质感 / 光影层次丰富 / 色彩鲜明）
#     - `negative_keywords`：反向关键词，规避负面效果（如 模糊 / 比例失调 / 色彩溢出 / 噪点过多 / 细节缺失）

# **输出要求**：
# 1.  JSON格式严格合规，字段层级清晰，无语法错误；
# 2.  所有描述**100%贴合图片实际**，不添加主观臆造内容，最小粒度拆分细节；
# 3.  语言简洁精准，符合AI绘图关键词表达习惯，可直接复制到AI绘图工具中使用。
#     """
